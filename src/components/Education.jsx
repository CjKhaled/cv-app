...
function Education({ school, handleSchoolChange, degree, handleDegreeChange, start, handleStartChange, end, handleEndChange, location, handleLocationChange, clearText, changeExistingEducationInfo }) {
    ...
    function saveAddSchool() {
        schools.push({school: school, degree: degree, start: start, end: end, location: location, id: counter});
        setCounter(counter + 1);
        cancelEdit();
    }

    function saveEditSchool() {
        schools[index] = {school: school, degree: degree, start: start, end: end, location: location, id: index}
        cancelEdit()
    }
    ...
    function editSchool(e) {
        ...
        changeExistingEducationInfo(schoolEdit.school, schoolEdit.degree, schoolEdit.start, schoolEdit.end, schoolEdit.location)
    }

    function showSchools() {
        return (
            <ul className="schools">
                {schools.map(schoolObj => <li onClick={editSchool} key={schoolObj.id} id={schoolObj.id}>{schoolObj.school}</li>)}
            </ul>
        )
    }
    ...
    function showDeleteButton() {
        return <button className="delete" onClick={deleteSchool}>Delete</button>
    }
    ...
    function showFormButtons() {
        return (
            <div className="form-buttons">
                ...
                <button className="save" onClick={add ? saveAddSchool : saveEditSchool}>Save</button>
            </div>
        )
    }

    function showAddEducationButton() {
        return <button className="add-education" onClick={addSchool}>Add Education</button>
    }
        ...
        return (
            <section className="container">
                <div className="collapsable-element">
                    ...
                    <button onClick={toggleEducationSection} className="open">{active ? 'Close' : 'Open'}</button>
                </div>
